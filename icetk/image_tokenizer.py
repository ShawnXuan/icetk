...
class ImageTokenizer(object):
    ...
    def encode(self, image_, l=1):
        ...
        img = self.tr_normalize(image_).to(self.device)
        if self.fp16:
            img = img.half()
        with flow.no_grad():
    ...
    def decode(self, codes, l=1):
        ...
        if isinstance(codes, list):
            codes = flow.tensor(codes, dtype=flow.long, device=self.device)
        ...
        with flow.no_grad():
            ...
            out = out * flow.tensor([0.30379, 0.32279, 0.32800], device=out.device).view(1, -1, 1, 1) + flow.tensor([0.79093, 0.76271, 0.75340], device=out.device).view(1, -1, 1, 1)
        return out
